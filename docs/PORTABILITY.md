...
OpenClaw plugins are pinned with an explicit `plugins.allow` list for the
bundled OpenClaw 2026.5.28 plugin set plus the bootstrapped Brave plugin.

Search defaults to the best configured provider in this order: Brave, Exa,
Tavily, Perplexity, Firecrawl, SearXNG, then DuckDuckGo. Set
`OPENCLAW_WEB_SEARCH_PROVIDER` to force a configured provider. When
`FIRECRAWL_API_KEY` is set, Firecrawl is also configured as the web fetch
fallback.
